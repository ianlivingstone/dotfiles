...
start_ssh_agent() {
    ...
    # Try to connect to existing agent. Only source it if WE own it — never source a
    # file another user could have planted (it sets env / could run code).
    local agent_env="$HOME/.ssh/agent-env"
    if [[ -f "$agent_env" && -O "$agent_env" ]]; then
    ...
    # Start new agent. Create agent-env as 0600 (umask 077) so the agent socket path
    # isn't world-readable on a shared machine.
    if ! (umask 077; ssh-agent > "$agent_env"); then
