...
validate_ssh_keys() {
    ...
    # NOTE: use process substitution, not `grep | while` — a pipeline runs the loop in a
    # subshell, so has_unencrypted would be lost and this check would never fail.
    ...
    done < <(grep "IdentityFile" "$xdg_config/ssh/machine.config")
...
validate_gpg_keys() {
    ...
    # Fall back to the real gpg default-key when there's no XDG machine.config —
    # otherwise the key actually used for signing would never get validated.
    if [[ -z "$selected_key" && -f "$HOME/.gnupg/gpg.conf" ]]; then
        selected_key=$(grep "^default-key" "$HOME/.gnupg/gpg.conf" 2>/dev/null | awk '{print $2}')
    fi

    if [[ -z "$selected_key" ]]; then
        return 0  # No key configured anywhere, nothing to validate
    fi
