...
configure_machine_keys() {
    ...
    if [[ -f "$gpg_template" ]]; then
        ...
        # Also generate gpg-agent.conf from template
        local agent_template="$DOTFILES_DIR/gnupg/gpg-agent.conf"
        local agent_config="$HOME/.gnupg/gpg-agent.conf"
        if [[ -f "$agent_template" ]]; then
            cp "$agent_template" "$agent_config"
            chmod 600 "$agent_config"
            echo -e "${GREEN}✅ GPG agent configuration generated${NC}"
        fi
    else
...
show_status() {
    ...
    # Temporarily disable ERR trap for sourced modules (they have different error handling)
    trap - ERR
